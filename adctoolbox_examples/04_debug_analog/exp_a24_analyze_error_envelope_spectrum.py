"""Error Envelope Spectrum Analysis: Sweep across 15 different ADC non-idealities

This experiment demonstrates error envelope spectrum analysis for various ADC impairments.
Uses Hilbert transform to extract error envelope and analyzes its spectrum directly to reveal
amplitude modulation patterns in the error signal.
"""
...
fig.suptitle(f'Error Envelope Spectrum Analysis: 15 ADC Non-idealities (Fs={params["Fs"]/1e6:.0f} MHz, Fin={params["Fin"]/1e6:.1f} MHz)',
             fontsize=14, fontweight='bold')
plt.tight_layout()

fig_path = (output_dir / 'exp_a24_analyze_error_envelope_spectrum.png').resolve()
