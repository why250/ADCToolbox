"""Error Autocorrelation Analysis: Sweep across 15 different ADC non-idealities

This experiment demonstrates error autocorrelation analysis for various ADC impairments.
Autocorrelation reveals temporal correlation patterns in the error signal, helping
distinguish between white noise and correlated errors.
"""
...
print("\n" + "=" * 100)
print(f"{'Case':<4} | {'Non-Ideality':<30} | {'ACF[0]':<10} | {'ACF[1]':<10} | {'ACF[10]':<10}")
print("-" * 100)
...
for idx, case in enumerate(CASES):
    ...
    # Print metrics
    print(f"{idx+1:<4} | {case['title']:<30} | {acf_0:>10.4f} | {acf_1:>10.4f} | {acf_10:>10.4f}")
...
fig.suptitle(f'Error Autocorrelation Analysis: 15 ADC Non-idealities (Fs={params["Fs"]/1e6:.0f} MHz, Fin={params["Fin"]/1e6:.1f} MHz)',
             fontsize=14, fontweight='bold')
plt.tight_layout()

fig_path = (output_dir / 'exp_a23_analyze_error_autocorrelation.png').resolve()
